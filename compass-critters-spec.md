...
- **Daily quests**: decided 2026-09-21, expanded to a rotating pool the same day after noticing the original 3 were fixed/static every day. A 5-entry pool - win 2 rounds (+2 coins), reveal 30 tiles (+2 coins), use a power-up (+2 coins), win a round without losing a life (+3 coins), correctly flag a critter (+2 coins) - 3 of which are picked fresh each calendar day (same `todayDateString()` boundary the streak uses, so it also respects the debug day-offset for testing), plus a +3 coin bonus for claiming all 3 of that day's picks. The 3 are chosen by a small seeded PRNG (`selectDailyQuestIds`) keyed on the date string, not `Math.random()` - deterministic per day (same 3 shown on every reload/reopen that day, no server round-trip needed) but effectively unpredictable day to day. Progress toward all 5 pool entries is tracked continuously regardless of which 3 are active that day (simpler than gating each tracking call by "is this quest live today"), but only the active 3 (plus the bonus) are ever shown or claimable. Progress accumulates automatically as the underlying stats change; coins are only granted when the player explicitly taps Claim in the Daily Quests panel, which also shows a small unclaimed-count badge on the button (e.g. "📅 Daily Quests (2)") as a lightweight nudge to check in. A save from before the pool expanded past its original 3 is backfilled with the 2 new entries (0 progress, unclaimed) the next time quest state is touched, without disturbing same-day progress already made on the original 3. Deliberately **no ad-watching quests** (unlike the reference screenshots that originally prompted this feature) - there's no real ad SDK integrated yet (see Monetization touchpoints), only objectives the game already has the data for.
