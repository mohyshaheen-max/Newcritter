...
## Retention: win-count milestones & daily quests

- Decided 2026-09-21, informed by a look at how other mobile puzzle games (Zoodoku's first-win/milestone badges and finite-campaign-into-endless-mode structure; daily-quest claim screens common across the genre) handle replayability. Two small, purely client-side/per-device systems, independent of the server-backed leaderboard/friends/gifting - these are personal habit loops, not competitive ones.
- **Win-count milestones**: a celebratory overlay the moment a lifetime win-count threshold is crossed - "First Win!" at win 1 (+3 coins), then every 5th win after that ("5 Wins!", "10 Wins!", ...) for +5 coins, indefinitely. Tracked by a new `totalWins` counter (persisted alongside the rest of the save, independent of level/tier progress), so it keeps paying out even once the 36-level ladder is fully cleared - one of the "other levers" the Difficulty progression section flags as needed for endless replay. Shown via a dedicated overlay chained after the win screen (and after the power-ups intro, if both happen to be pending the same win): win overlay → [power-ups intro, if pending] → [milestone, if pending] → next round starts.
...
- Both are considered a first pass, not the final word on retention - the earlier Zoodoku-style research also surfaced a genuine daily *puzzle* (one seeded board per day, shareable/comparable across players) and an explicitly-branded "Endless Mode" past the ladder as further options, intentionally deferred pending a decision on whether the daily puzzle should be server-seeded (comparable leaderboard entry, more Worker/D1 work) or per-device (free, but not comparable).
...
- [ ] Branded/expanded "Endless Mode" past level 36 — currently just replays 8×8/8-critters indefinitely with no distinct framing or added variety
