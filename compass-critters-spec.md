...
- Decided 2026-09-21, expanded same day per feedback that the first cut was too thin — a scripted interactive tutorial, not a slide-deck explainer: a new player's very first taps land on a small fixed 5×5 demo board (critters at opposite corners, entirely separate from the real tier ladder/coins/persistence) rendered with the actual grid CSS and gesture handling, so what they learn transfers directly. Eight guided steps covering the whole core loop except power-ups (see below): a plain arrow; a red/near arrow (within 2 tiles); a ✦ tie; an explanatory step on masking (why a farther critter stays invisible until you're closer to it than to anything else); flagging a tile via hold/right-click; tapping a critter itself (life cost, Continue offer at 0); the win condition; and a closing note on coins-from-stars and the daily streak. Interactive steps highlight exactly one target tile (or, for the flag step, require the hold/right-click gesture specifically, not a plain tap) - action on any other tile, or the wrong gesture, is simply ignored, not blocked or shaken. Purely explanatory steps show their text immediately with no tile interaction required.
