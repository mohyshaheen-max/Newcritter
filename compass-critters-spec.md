...
- Decided 2026-09-22, revised 2026-09-23, win-count trimmed 2026-09-21 (3→2 wins per level, see below): within each grid size N (3×3 through 8×8), critter count ramps up, 2 wins per level before advancing (no demotion on a loss - a loss just retries the same level). Displayed to the player as "Level" (e.g. "Level 1/18"); internally still called a tier in code/variable names.
...
- [x] Levels-per-grid-size pacing — superseded 2026-09-23 by the 18-level critter-count ramp, win count trimmed 2026-09-21 (see Difficulty progression): each size's top 3 critter counts only, 2 wins per level, no demotion on a loss
